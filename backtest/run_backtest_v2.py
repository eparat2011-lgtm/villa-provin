...
# ─── BOT 2: EMA 3/8 + RSI, 3min (resampled from 1min) ────────────────────────

def run_bot2(symbol, df_1m_full):
    """Bot 2 — Scalper (EMA 3/8, 3min)."""
    df_work = df_1m_full.copy()
    df_work_indexed = df_work.set_index("datetime")
    
    df_3m = df_work_indexed.resample("3min").agg({
        "open": "first",
        "high": "max",
        "low": "min",
        "close": "last",
    ...
    df = df_3m.copy()
    df["ema3"] = ema(df["close"], 8)
    df["ema8"] = ema(df["close"], 21)
    df["rsi"] = rsi_calc(df["close"], 14)
    ...
    for i in range(1, len(df) - 1):
        ...
        ema3_prev = df["ema3"].iloc[i - 1]
        ema8_prev = df["ema8"].iloc[i - 1]
        ema3_curr = df["ema3"].iloc[i]
        ...
        cross_up = (ema3_prev <= ema8_prev) and (ema3_curr > ema8_curr)
        if cross_up and 48 <= rsi_curr <= 65:
            signals.append((i, "long"))
        
        cross_down = (ema3_prev >= ema8_prev) and (ema3_curr < ema8_curr)
        if cross_down and 35 <= rsi_curr <= 52:
            signals.append((i, "short"))
    
    trades = simulate_trades(df, signals, tp_pct=0.005, sl_pct=0.003,
                              position_size=100, leverage=10, lookahead=100)
    return trades
...
def main():
    ...
    def recommend(s1, s2, s3, best):
        ...
        for s, name, desc in [
            ...
            (s2, "Bot 2 (Scalper EMA 8/21)", "ranging/choppy markets"),
...
{fmt_stats(s2, "⚡", "Bot 2 — Scalper (EMA 8/21, 3min)")}
