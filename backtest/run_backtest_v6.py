...
def run_bot2(symbol, df_1m_full):
    ...
    for i in range(22, len(df) - 1):
        ...
        # Long: price at/below lower BB + RSI < 35 + SAR uptrend
        if price <= bb_lower * 1.001 and rsi_val < 35 and sar_uptrend:
            signals.append((i, "long"))
        
        # Short: price at/above upper BB + RSI > 65 + SAR downtrend
        if price >= bb_upper * 0.999 and rsi_val > 65 and not sar_uptrend:
            signals.append((i, "short"))
...
def main():
    ...
    def recommend(s1, s2, s3, best):
        ...
        for s, name, desc in [
            ...
            (s2, "Bot 2 (Regime-Aware Mean Reversion)", "oversold/overbought reversals"),
